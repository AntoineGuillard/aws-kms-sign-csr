...
def sign_certification_request_info(kms, key_id, csr, digest_algorithm, signing_algorithm):
    ...
    response = kms.sign(KeyId=key_id, Message=digest,
                        MessageType='DIGEST',
...
def main(args):
    with open(args.csr, 'r', encoding='utf-8')  as csr_file:
        substrate = pyasn1_modules.pem.readPemFromFile(csr_file, startMarker=START_MARKER,
                                                       endMarker=END_MARKER)
        csr = decoder.decode(substrate,
                             asn1Spec=pyasn1_modules.rfc2986.CertificationRequest())[0]
        if not csr:
    ...
    signature_bytes = sign_certification_request_info(kms, args.keyid, csr, args.hashalgo,
    ...
    sig_algo_identifier.setComponentByName('algorithm',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hashalgo', choices=['sha224', 'sha256', 'sha512', 'sha384'],
                        default="sha256", help='hash algorithm to choose')
    parser.add_argument('--signalgo', choices=['ECDSA', 'RSA'], default="RSA",
